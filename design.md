...
**参考文档**：
- `design_mapping.md` 维护 Reed 与 libCEED 的概念、对象和数据类型映射，用于后续功能对标与示例迁移。
...
let my_qf = reed.q_function_interior(
    ...
    vec![ /* QFunctionField inputs */ ],
    vec![ /* QFunctionField outputs */ ],
    0, // QFunctionContext size in bytes (match OperatorBuilder::qfunction_context when > 0)
    Box::new(|_ctx: &[u8], q: usize, inputs: &[&[f64]], outputs: &mut [&mut [f64]]| {
        let u = inputs[0];
