...
fn run_combined(dim: usize, nelem_1d: usize, p: usize, q: usize) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let r_q_mass = reed.strided_elem_restriction(
        nelem,
        qpts_per_elem,
        1,
        nelem * qpts_per_elem,
    ...
    let mut qdata_mass = reed.vector(nelem * qpts_per_elem)?;
    qdata_mass.set_value(0.0)?;

    let op_build_mass = reed
        .operator_builder()
        .qfunction(reed.q_function_by_name(qf_mass_build)?)
    ...
    let (r_q_poisson, qdata_poisson) = if dim == 1 {
        ...
        let mut qdata_vec = reed.vector(nelem * qpts_per_elem * qdata_comp)?;
        qdata_vec.set_value(0.0)?;

        let op_build_poisson = reed
            .operator_builder()
            .qfunction(reed.q_function_by_name(qf_poisson_build_opt.expect("poisson build name"))?)
            .field("dx", Some(&*r_x), Some(&*b_x), FieldVector::Active)
            .field("weights", None, Some(&*b_x), FieldVector::None)
            .field("qdata", Some(&*r_q), None, FieldVector::Active)
            .build()?;
    ...
    println!("libCEED ex3-volume (Reed {dim}D)");
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.iter().any(|a| a == "-h" || a == "--help") {
        println!("Usage: cargo run --example ex3_volume_combined -- [--dim D] [--nelem N] [--p P] [--q Q]");
